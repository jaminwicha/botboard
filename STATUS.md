...
| §3.2 stateful effects (HP/armor/ammo), spawn/utility, lasers | 🔲 Phase 3+ | SoA state arrays + state-bucket hashing reserved (`zobrist.rs` BUCKETS) |
| §3.3 Special moves — castling, en passant, double-step; drops with 3 legality tiers (*nifu*, *uchifuzume*) | ✅ | `movegen.rs` |
| §3.4 Turn model — one turn = one action, strict rotation, pass forbidden by default; compound moves | ✅ base rule; compounds represented as single `Move` (aux/script slots), generator arrives with Phase-3 abilities | `moves.rs`, `game.rs` (TurnPolicy/PassPolicy) |
...
| §7.5 Zobrist w/ state buckets, hands, stm; repetition = full-state equality; terrain keys | ✅ (terrain keys activate with terrain) | `zobrist.rs`, `search.rs`, `selfplay.rs` |
...
| §12 Prototype 1 (representation crossover benchmark) | 🔲 pending | needs SIMD-bitboard second class |
...
| §3 Reconciliation (GT-CFR / AlphaZero / R-NaD family) | ◐ ladder rungs share the one evaluator; regret-matching solver lives in `league.rs::nash_averaging`; full GT-CFR/NeuRD trainers = the flagged load-bearing prototype | |
...
| §6 Gate training | 🔲 labels-from-rung-agreement pending; threshold gate shipped | `ladder.rs` |
